...
fn test_initiate_transfer() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			// event is emitted
			assert_event_emitted(RuntimeEvent::ISO8583(crate::Event::<Test>::InitiateTransfer {
				from: 3,
				to: 10,
				amount: 100,
			}));
...
fn test_submit_finalities() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_event_emitted(RuntimeEvent::Balances(pallet_balances::Event::<Test>::Deposit {
				who: 4,
				amount: 20,
			}));
